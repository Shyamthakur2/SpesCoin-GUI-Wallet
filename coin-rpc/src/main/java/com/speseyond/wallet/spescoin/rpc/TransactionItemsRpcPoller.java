...
public class TransactionItemsRpcPoller extends RpcPoller<TransactionItems> implements Observer {
	...
	public String getParams() {
		...
		if (this.addresses.getAddresses().size() > 0) {
			params = "\"params\":{\n" + "    \"firstBlockIndex\":" + firstBlockCount + ",\n" + "    \"blockCount\":" + knowBlockCount + "," + "    \"addresses\":[\n";
	...
	public void updateOnSucceed(TransactionItems data) {
		// if (data.getItems().get(data.getItems().size() - 1).getTransactions().size()
		// > 1) {
		// for (Transaction item : data.getItems().get(data.getItems().size() -
		// 1).getTransactions()) {
		...
		// }
		// } else {
		// firstBlockCount = data.getItems().get(data.getItems().size() -
		// 1).getTransactions().get(0).getBlockIndex();
		// }
